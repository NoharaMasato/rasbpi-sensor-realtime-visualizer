...
cur = conn.cursor() 
cur.execute('select id from airs order by id desc limit 1')
airs_record_num = cur.fetchall()
airs_db_id = airs_record_num[0][0] + 1

cur.execute('select id from accelerations order by id desc limit 1')
accelerations_record_num = cur.fetchall()
accelerations_db_id = accelerations_record_num[0][0] + 1

cur.execute('select id from geomagnetics order by id desc limit 1')
geomagnetics_record_num = cur.fetchall()
geomagnetics_db_id = geomagnetics_record_num[0][0] + 1
...
class NtfyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data): 
        ...
        global airs_db_id
        global accelerations_db_id 
        global geomagnetics_db_id
        # breakpoint()
        if int((cal[0:2]), 16) == 0xf2:
            ...
            cur.execute('INSERT INTO geomagnetics (GeoMagnetic_X,GeoMagnetic_Y,GeoMagnetic_Z,id) VALUES ({0},{1},{2},{3})'.format(GeoMagnetic_X,GeoMagnetic_Y,GeoMagnetic_Z,geomagnetics_db_id))
            conn.commit()
            ...
            cur.execute('INSERT INTO accelerations (Acceleration_X,Acceleration_Y,Acceleration_Z,id) VALUES ({0},{1},{2},{3})'.format(Acceleration_X, Acceleration_Y, Acceleration_Z,accelerations_db_id))
            conn.commit()
            accelerations_db_id += 1
            geomagnetics_db_id += 1
        ...
        if int((cal[0:2]), 16) == 0xf3:
            ...
            cur.execute('INSERT INTO airs (Pressure,Humidity,Temperature,UV,AmbientLight,id) VALUES ({0},{1},{2},{3},{4},{5})'.format(Pressure,Humidity,Temperature,UV,AmbientLight,airs_db_id))
            conn.commit()
            airs_db_id += 1
...
def main():
    alps = AlpsSensor("48:F0:7B:78:4B:7A")
